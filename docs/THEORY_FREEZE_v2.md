...
Canonical workflow authority at freeze: `ryotamatsuki/research-paper-workflow` v1.1, release SHA `488e5ab06c207909296a7564eaf9066f7f94319c`.

Stage 7.5R authorization source: commit `7c7f19094c0151f350e0b256152c34da6ba9f851`.

## Historical note

This file preserves the v2 freeze exactly as the theory authority used through Stage 10R, except for this supersession header. Stage 11 subsequently found one bounded defect in P2R: v2 stated pointwise strict derivative signs for arbitrary differentiable increasing strictly concave `g`, while its assumptions only support monotone ordering without additional smoothness. The quadratic baseline, P1, P3R, P4, P5R, welfare formulas, price continuation, and contribution boundary were not rejected. The active repaired freeze is `docs/THEORY_FREEZE.md` with identifier `SSDI-THEORY-FREEZE-2026-09-06-v3`.
...
Status in v2: `PROVED` after repair, later found overstated by Stage 11.

For differentiable increasing strictly concave `g` on `[0,E]`, the private maximizer is globally nonincreasing in `b`, while the coordinated symmetric maximizer is globally nondecreasing in `b`. v2 additionally stated that strict opposite derivative signs hold on intervals where the relevant optima are interior. Stage 11 rejected only that pointwise derivative formulation under the stated smoothness assumptions.
...
- a negative endogenous innovation-composition term caused by `dx^F/db<0` in the quadratic baseline.
...
Only the following robustness statements were authorized in v2:

1. General increasing strictly concave `g`: weak directional monotonicity globally, with the later-rejected pointwise strict derivative statement conditional on interior optima.
...
The following were explicitly outside the v2 freeze:
...
Any theoretical change after this freeze must record what changed, why, affected equations/propositions, verification artifacts, literature/contribution claims, and the earliest workflow stage reopened.
