...
Active freeze after Stage-7R2 repair: `SSDI-THEORY-FREEZE-2026-09-06-v3`

Workflow: `ryotamatsuki/research-paper-workflow` v1.3, release commit `3e4e6a3f76d86058024d06f9710f942e21627386`.

This record implements the mandatory Stage-10 Figure/Table Architecture Gate. Stage 7R2 subsequently repaired only the formulation of the general-technology P2R result; the quadratic policy theorem and this exposition architecture are unchanged and have been revalidated under v3.
...
| General private/coordinated directional wedge | for `b2>b1`, private `x^F(b2)<=x^F(b1)` and coordinated `x^S(b2)>=x^S(b1)` globally; strict ordering when both compared optima are interior | Proposition 2 | The key content is an order/domain distinction; a calibration plot could obscure corners or suggest an unsupported pointwise derivative claim | `docs/THEORY_FREEZE.md`; Appendix order-comparative-statics proof | Yes |
...
| Welfare decomposition | direct diffusion gain plus an endogenous portfolio term that is zero at `b=0` and negative for `b>0` in the quadratic interior baseline | Equation plus concise prose | The decomposition has two signed terms and no non-monotone path requiring a visual | Section 4; `scripts/symbolic_verify.py` | Yes |
...
| General-technology robustness boundary | weak global ordering plus strict interior ordering; no general pointwise derivative or policy-curvature theorem | Concise prose anchored to Proposition 2 | The contribution is the limit on the claim, not a quantitative sensitivity exercise | v3 freeze and Stage-7R2 repair record | Yes |
| Incomplete transferability | effective transferability `t=lambda b` preserves relative-return wedge | Concise prose | Reparameterization is algebraic; visual is redundant | v3 freeze; Section 5 | Yes |
| Endogenous total R&D | only conditional relative FOC `(1-nu b)g'(x)=g'(z)` when both activities face the same marginal total-capacity cost | Concise prose | No policy-persistence theorem is authorized, so a quantitative extension plot would overstate evidence | v3 freeze; Section 5 | Yes |
...
1. **Private-versus-coordinated allocation-path figure — rejected.** A single calibration could visually suggest a general pointwise derivative claim. Proposition 2 communicates the correct order quantifiers more precisely.
...
3. **Sensitivity table — rejected.** No multi-parameter robustness theorem beyond the frozen threshold is authorized.
...
`FIGURE/TABLE ARCHITECTURE COMPLETE — REVALIDATED UNDER FREEZE v3`

Every headline result has a primary exposition vehicle. The only required quantitative visual remains reproducibly implemented; no quantitative table is required.
